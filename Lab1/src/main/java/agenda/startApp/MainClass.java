...
public class MainClass {
		...
		try {
			...
			while (chosen != 4) {
				...
				try {
					switch (chosen) {
					case 1:
						addContact(contactService, in);
						break;
					case 2:
						addActivity(activityService, in, user);
						break;
					case 3:
						showActivity(activityService, in, user);
						break;
					}
				} catch (Exception e) {
	...
	private static void showActivity(ActivityService activityService,
	...
	private static void addActivity(ActivityService activityService,
			BufferedReader in, User user) {
		try {
